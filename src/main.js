...
function createSidebar() {
  data.forEach((item, index) => {
    ...
    sphereItem.textContent = `${item.name}`;
...
function highlightSphere(index, highlight) {
  ...
  if (highlight) {
    gsap.to(sphere.scale, { x: 1.1, y: 1.1, z: 1.1, duration: 0.3 });
...
function transitionToDetailScene(bubble) {
  ...
  detailScene = new DetailScene(
    bubbleData,
    bubbleColor,
    transitionConfig.detailSceneBubbles,
    transitionToMainScene
  );
...
function updateVisualization(newData) {
  ...
  mainScene.updateData(data);

  // Update the sidebar
  updateSidebar();
}

setupWebSocket(updateVisualization);
